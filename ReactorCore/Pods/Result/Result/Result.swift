...
public func materialize<T>(_: () throws -> T) -> Result<T, NSError> {
    fatalError()
}

@available(*, unavailable, message: "Use the overload which returns `Result<T, AnyError>` instead")
public func materialize<T>(_: @autoclosure () throws -> T) -> Result<T, NSError> {
    fatalError()
}
    ...
    public func `try`<T>(_: String = #function, file _: String = #file, line _: Int = #line, try _: (NSErrorPointer) -> T?) -> Result<T, NSError> {
        fatalError()
    }
    ...
    @available(*, unavailable, message: "This has been removed. Use `Result.init(attempt:)` instead. See https://github.com/antitypical/Result/issues/85 for the details.")
    public func `try`(_: String = #function, file _: String = #file, line _: Int = #line, try _: (NSErrorPointer) -> Bool) -> Result<(), NSError> {
        fatalError()
    }
